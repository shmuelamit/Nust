...
pub fn instr_plp(cpu: &mut Cpu, _mode: AddresingMode) {
...
pub fn instr_brk(cpu: &mut Cpu, _mode: AddresingMode) {
...
pub fn instr_rti(cpu: &mut Cpu, _mode: AddresingMode) {
    // Panic shouldn't happen because we have all flag possibilities
    cpu.status = CpuFlags::from_bits(cpu.stack_pop()).unwrap();
    cpu.status.remove(CpuFlags::BS | CpuFlags::B);
...
pub fn instr_rts(cpu: &mut Cpu, _mode: AddresingMode) {
